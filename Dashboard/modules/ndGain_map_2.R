...
observeEvent(input$comparison_country_search_map_2, {
  req(input$comparison_country_search_map_2)
  country <- input$comparison_country_search_map_2
  countryND_map_2(country)
  zoom_to_country_nd("nd_gain_map_2", country)
})

observeEvent(c(input$nd_year_map_2,
               input$variable_nd_map_2,
               input$comparison_country_search_map_2), {
                 req(input$nd_year_map_2)
                 req(input$variable_nd_map_2)
                 req(input$comparison_country_search_map_2)
                 country <- input$comparison_country_search_map_2
                 countryND_map_2(country)
                 data <- gain %>%
                   select(ISO3, Name, Year, input$variable_nd_map_2) %>%
                   filter(Year == input$nd_year_map_2)
                 
                 score <- data %>%
                   filter(Name == country) %>%
                   pull(input$variable_nd_map_2)
                 
                 # add it here
                 year <- as.character(input$nd_year_map_2)
                 pointData <- gain_wide_points %>%
                   filter(Name == country) %>%
                   select(name_en, iso_a3.x, matches(year))
                 
                 nd_year_score_map_2(score)
                 nd_year_data_map_2(data)
                 year_map_2(input$nd_year_map_2)
                 point_data_map_2(pointData)
               }
)

observeEvent(c(input$manual_min_input_map_2, input$manual_max_input_map_2), {
  req(input$manual_min_input_map_2, input$manual_max_input_map_2)
  updateSliderInput(
    session,
    "value_range_map_2",
    value = c(input$manual_min_input_map_2, input$manual_max_input_map_2)
  )
})
...
output$nd_gain_map_2 <- renderLeaflet({
  ...
  year_data <- gain %>%
    filter(Year == year)
  ndVar <- input$variable_nd_map_2
  pal <- colorNumeric(
    palette = "Purples",  
    domain = c(min_val_nd_map_2(), max_val_nd_map_2())
    )
...
observe({
  ...
  min_val_nd_map_2(min(valid_vals))
  max_val_nd_map_2(max(valid_vals))
  
  pal <- colorNumeric("Purples", domain = data[[input$variable_nd_map_2]])
  label <- gainVarsNames[gainVars == input$variable_nd_map_2]
  ...
  if (!is.null(countryND_map_2()) && countryND_map_2() != "" && countryND_map_2() != "Global (Default)") {
    zoom_to_country_nd("nd_gain_map_2", countryND_map_2())
  }
  
  selected_iso <- if (!is.null(countryND_map_2())) {
    iso3 <- gain %>%
      filter(Name == countryND_map_2()) %>%
      pull(ISO3) %>%
      unique()
  ...
  proxy %>% addPolygons(
    ...
    fillColor = ~pal(get(input$variable_nd_map_2)),
    fillOpacity = 0.8,
    color = "white",
    weight = 1,
    smoothFactor = 0.5,
    label = ~paste0(Name, ": ", round(get(input$variable_nd_map_2), 4)),
    layerId = ~iso_a3,
...
#   req(countryND_map_2(), nzchar(countryND_map_2()))
#   
#   filtered <- gain %>%
#     filter(Name == countryND_map_2())
#   
...
#     labs(title = paste0(label, " for ", countryND_map_2(), " (1995-2022)"),
...
observeEvent(input$variable_nd_map_2, {
  req(varND_map_2())
  var <- varND_map_2()
...
output$nd_year_map_2_score <- renderText({
...
observeEvent(input$nd_gain_map_shape_click, {
  ...
  updateTextInput(session, "comparison_country_search_map_2", value = clicked_country)
})